...
function frame(t: number, P: Params, out: Dot[]) {
  ...
  for (let i = 0; i < n; i += 1) {
    const q = fib(i, n)
    const m = Math.max(1e-4, Math.max(Math.abs(q[0]), Math.abs(q[1]), Math.abs(q[2])))
    const s = 1 + (1 / m - 1) * k * 0.62
    out.push(spin([q[0] * s, q[1] * s, q[2] * s, 0.85, 0.9], TAU * t, 0.42))
  }
}
...
export default function OrbCube({
  style,
  className,
  dotColor = "#3B82F6",
  density = 300,
